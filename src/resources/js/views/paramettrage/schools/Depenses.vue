      ...
      <div class="row justify-content-end">
        <div class="col-md-3">
          <div class="form-group">
            <label class="form-label"> Managers (cs/sales) </label>

            <vue-select
              name="account_manager"
              :options="managers"
              :reduce="(accMan) => accMan"
              label="nom"
              v-model="manager"
  ...
  methods: {
    ...
    getByManager() {
      ...
      if (!this.manager) {
        ...
        this.manager.role == 'Sales Manager' ? this.state = "lead" : this.state = "null";
        if (token) {
          axios
            .get("/api/get/depenses/by_filter/" + this.manager.id + "/" + this.state, {
              headers: {
                Authorization: "Bearer " + token,
              },
            })
    ...
    getByRubrique() {
      ...
      if (this.rubrique == "") {
        ...
        if (token) {
          axios
            .get("/api/get/depenses/by_filter_rubrique/" + this.rubrique, {
              headers: {
                Authorization: "Bearer " + token,
              },
            })
