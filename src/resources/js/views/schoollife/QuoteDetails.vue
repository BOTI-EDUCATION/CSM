            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h2 class="mb-0">{{ quote.Author }} {{"|"}} {{ quote.function }}
                        </h2>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-12 col-md-4">
                                <h4> Image : </h4>
                                <img :src="quote.image" alt="" class="img-fluids">
                            </div>
                            <div class="col-12 col-md-8" v-html="quote.text"></div>
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        getQuote: async function (postId) {
            // const token = localStorage.getItem("auth-token");
            // if (postId && token) {
                await axios
                    .get("/api/getQuoteDetails/" + postId)
                    .then(async (result) => {
